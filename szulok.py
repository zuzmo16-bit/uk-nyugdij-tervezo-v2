...
st.title("🛡️ Perennis: A Birodalmi Vagyonkezelő (Master Edition v2.2)")
st.write("Stratégia: SIPP Meltdown -> ISA felélés -> Holding hízlalás a maximumig.")
...
if user_mode == "Órabéres alkalmazott":
    ...
    working_years = st.sidebar.slider("Hány évig dolgozol még?", 0, int(75-current_age), 36)
...
if gross_sipp_meltdown <= 4189:
    ...
    st.sidebar.markdown('<div style="background-color: #d1ecf1; padding: 10px; border-radius: 5px; border-left: 5px solid #007bff;"><b>🔵 40%-os sáv (Higher Rate)</b></div>', unsafe_allow_html=True)
else:
    st.sidebar.markdown('<div style="background-color: #f8d7da; padding: 10px; border-radius: 5px; border-left: 5px solid #dc3545;"><b>🔴 45%-os sáv</b></div>', unsafe_allow_html=True)
...
for m in range(int((death_age - current_age) * 12) + 1):
    ...
    # 1. Befizetés 57-ig (Mindent SIPP-be)
    ...
    # 4. Meltdown & Megélhetés prioritás
    ...
    if age >= drawdown_start_age:
        ...
        if total_pension > 0:
            ...
            # Maradvány kezelés (SIPP hízlalja az ISA-t és Holdingot)
            ...
            if net_after_essentials > 0:
                ...
                # HIÁNY: Itt jön a stratégiai váltás: Előbb az ISA-t éljük fel!
                shortfall = abs(net_after_essentials)
                if current_isa >= shortfall:
                    current_isa -= shortfall
                else:
                    shortfall -= current_isa
                    current_isa = 0
                    current_holding = max(0, current_holding - shortfall)
        else:
            # SIPP elfogyott: State Pension -> ISA -> Holding sorrend
...
st.info("**Stratégia:** A SIPP kifogyása után a rendszer automatikusan az ISA-t kezdi el apasztani, hogy a Holdingban (Birodalmi tőke) lévő vagyon a lehető legtovább érintetlenül kamatozhasson.")
